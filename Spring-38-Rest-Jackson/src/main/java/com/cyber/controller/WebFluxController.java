...
import com.cyber.repository.GenreRepository;
...
public class WebFluxController {
    ...
    //flux - bcs we retrieve multiple movie-cinemas
    @GetMapping("/flux-movie-cinemas")
    public Flux<MovieCinema> readAllCinema(){
        return Flux.fromIterable(movieCinemaRepository.findAll());
    }

    //mono - bcs we retrieve only one movie-cinema - with path variable
    @GetMapping("/mono-movie-cinema/{id}")
    public Mono<MovieCinema> readById(@PathVariable("id") Long id){
        return Mono.just(movieCinemaRepository.findById(id).get());
    }

    //mono - bcs we retrieve only one movie-cinema - with request param
...
//        return Mono.just(updatedGenre);
    ...
    //------------------- WEBCLIENT EXAMPLE -----------------//
    //now it is time to write a webclient to consume above APIs

    @GetMapping("/flux") // --> this api consumes this api: "/flux-movie-cinemas"
